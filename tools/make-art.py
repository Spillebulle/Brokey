...
def main() -> int:
    ...
    # These four, and deliberately not crates/brokey/icons/icon.ico, which
    # this script does not write. That file is the one tauri-build compiles
    # into brokey.exe as its RT_GROUP_ICON, and the MSI's Start menu shortcut
    # is a plain .lnk that takes its icon out of the executable rather than
    # out of the package, so icon.ico is the Start menu and taskbar icon.
    # assets/icons/brokey.ico above *is* written here, and it is the one the
    # MSI uses for the Add/Remove Programs entry. So changing the accent hue
    # in tokens.css and rerunning this script moves the Add/Remove Programs
    # icon and leaves the Start menu one at the old hue, until icon.ico is
    # regenerated by hand from the PNGs this script has just written.
