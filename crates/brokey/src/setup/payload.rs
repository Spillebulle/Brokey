...
//! |  brokey-x.y.z.msi  |
//! +-------------------+
//! | BROKPKG | len u64 |   <- 16 bytes, little-endian
//! +-------------------+
//! ```
//!
...
//! after the last section are simply ignored: `brokey-setup.exe` runs as Brokey
...
///
/// Brokey's MSI is about seven megabytes. The bound is not about the real
/// file: it is what stops a corrupt or hostile footer claiming four
/// exabytes and being believed all the way into a slice.
...
mod tests {
    ...
    fn scratch(name: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("brokey-payload-{}", std::process::id()));
    ...
    ///
    /// Setup is double-clicked, so it is launched with no command line at
    /// all: dispatching on an argument would leave a setup executable with
    /// no way to reach its own installer. The thing this pins is that the
    ...
    fn a_file_carrying_a_package_is_recognised_by_its_last_bytes_alone() {
        ...
        // And the ordinary case, which is every launch of Brokey itself.
        let plain = scratch("brokey.exe");
    ...
    /// The ordinary case: `brokey.exe` has no payload and must not be read as
    /// though it had one.
    #[test]
    ...
    fn the_magic_is_brokeys_own() {
        ...
        assert_eq!(
            MAGIC.len(),
            8,
            "eight bytes, so a plausible length is not mistaken for one"
        );
    }
}
